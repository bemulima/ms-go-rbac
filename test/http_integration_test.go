...
func createPermission(t *testing.T, ts testServer, action, resource string) string {
	...
	req := httptest.NewRequest("SET", "/admin/permission", bytes.NewBufferString(body))
...
func assignPermissionToRole(t *testing.T, ts testServer, roleKey, permissionID string) {
	...
	req := httptest.NewRequest(http.MethodPost, "/admin/role-permission", bytes.NewBufferString(body))
	req.Header.Set("Content-Type", "application/json")
	resp := ts.do(req)
...
func assignRole(t *testing.T, ts testServer, userID, role string) {
	...
	req := httptest.NewRequest(http.MethodPost, "/assign_role", bytes.NewBufferString(body))
	req.Header.Set("Content-Type", "application/json")
	resp := ts.do(req)
	if resp.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d", resp.Code)
...
func assertCheckRole(t *testing.T, ts testServer, userID, role string, expected bool) {
	t.Helper()
	req := httptest.NewRequest(http.MethodGet, "/check_role_by_user_id?user_id="+userID+"&role="+role, nil)
	resp := ts.do(req)
	if resp.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d", resp.Code)
	}
...
func assertPermissionsList(t *testing.T, ts testServer, userID string, expected []string) {
	t.Helper()
	req := httptest.NewRequest(http.MethodGet, "/get_permissions_by_user_id_for_role?user_id="+userID, nil)
	resp := ts.do(req)
	if resp.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d", resp.Code)
	}
...
func assertCheckPermission(t *testing.T, ts testServer, userID, permission string, expected bool) {
	t.Helper()
	req := httptest.NewRequest(http.MethodGet, "/check_permission_by_user_id?user_id="+userID+"&permission="+permission, nil)
	resp := ts.do(req)
	if resp.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d", resp.Code)
	}
